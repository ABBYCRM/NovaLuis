...
const EXPECTED_SHA = String(process.env.EXPECTED_SHA || process.env.GITHUB_SHA || "").trim();
...
async function fetchRetry(url, options = {}, attempts = 12) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      lastError = new Error(`${url} returned HTTP ${response.status}: ${text.slice(0, 500)}`);
    } catch (error) {
      lastError = error;
    }
...
async function verifyApiRuntime() {
  const [health, openclaw, cron, skills, evidenceSkill, workTree] = await Promise.all([
  ...
  if (openclaw.status !== "ready") throw new Error(`OpenClaw status is ${openclaw.status}`);
  if (cron.running !== true) throw new Error("embedded social cron is not running");
  if (cron.publicBaseUrlConfigured !== true) throw new Error("PUBLIC_BASE_URL is not HTTPS");
  if (cron.publicBaseUrl !== LIVE_URL) throw new Error(`unexpected PUBLIC_BASE_URL: ${cron.publicBaseUrl}`);
  if (cron.embeddedWorker !== true) throw new Error("embedded social worker is not active");
  if (cron.siblingWorkerEnabled !== true) throw new Error("sibling social worker is disabled");
  if (!Array.isArray(workTree.runs)) throw new Error("work-tree route did not return a runs array");
  ...
  for (const name of requiredSkills) {
    ...
    if (skill.source !== "workspace") throw new Error(`${name} is not sourced from workspace skills`);
  }
  if (evidenceSkill.name !== "evidence-first-execution") throw new Error("wrong evidence skill detail");
  ...
  const missingResponse = await fetch(missingAsset, { signal: AbortSignal.timeout(20_000) });
  const missingBody = await missingResponse.text();
  if (missingResponse.status !== 404 || !missingBody.includes("asset not found")) {
    throw new Error(`hardened Instagram asset boundary returned ${missingResponse.status}: ${missingBody.slice(0, 300)}`);
  }
...
async function verifyPwaAndMobileAssets() {
  ...
  const [html, manifest, sw, navigation, voice, durable, icon192, icon512] = await Promise.all([
    text(`${LIVE_URL}/`),
    ...
    fetchRetry(`${LIVE_URL}/icon-192.png?v=${version}`).then((response) => response.arrayBuffer()),
    fetchRetry(`${LIVE_URL}/icon-512.png?v=${version}`).then((response) => response.arrayBuffer()),
  ]);
  ...
  if (!navigation.includes("/assets/continuous-voice-input.js")) throw new Error("voice module loader missing");
  if (!navigation.includes("/assets/durable-run-reconcile.js")) throw new Error("durable module loader missing");
  if (!navigation.includes("versionQuery")) throw new Error("child runtime modules are not versioned");
  if (!voice.includes("instance.continuous = true")) throw new Error("continuous microphone runtime missing");
  if (!voice.includes("window.__novaVoiceInput")) throw new Error("voice observability handle missing");
  if (!durable.includes("/api/work-tree/runs/")) throw new Error("durable reconciliation endpoint missing");
  if (!durable.includes("[NOVA_RUN_ID:")) throw new Error("durable run marker missing");
  if (icon192.byteLength < 100 || icon512.byteLength < 100) throw new Error("PWA icon payload is empty");
