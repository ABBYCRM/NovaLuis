...
Usage:
  python3 scripts/render_scenarios/generate.py
  # Output: scripts/render_scenarios/render_scenarios.csv
"""
import csv
import os
import random
...
OUT_PATH = str(SCRIPT_DIR / "render_scenarios.csv")
TARGET_ROWS = 500
...
SEVERITY_BY_CATEGORY = {
    "build": "high", "boot": "high", "runtime": "medium", "scaling": "medium",
    "env": "high", "api": "medium", "domain": "medium", "data": "high",
    "recovery": "critical", "workflow": "medium",
}
    ...
    for cat in cat_order:
        ...
        for ti, t in enumerate(triggers):
            for ci, c in enumerate(conds):
                ...
                produced += 1
                rows.append({
                    "id": f"RS-{counter:04d}", "category": cat, "trigger": t,
                    "condition": c, "if_action": if_action, "else_action": else_action,
                    "severity": SEVERITY_BY_CATEGORY[cat], "source_doc": SOURCE_DOCS[cat],
                })
            if produced >= want:
                break
        if produced < want:
