...
export const RESPONSE_FORMAT_CONTRACT = [
  ...
  "Use a descriptive heading, short labeled sections, numbered records, and one fact per line.",
  ...
  "Use this contact-card order when fields exist: bold numbered name; 🏢 company; 💼 role; 🔗 profile URL; 📧 linked email; 📱 phone; then a restrained status line.",
  "Separate records with a horizontal rule. Do not present the same records again in a second table, code block, or duplicate list.",
  "After a contact list, add a '### 💬 Outreach Draft' blockquote only when outreach copy is relevant, a '### 📊 Current Pipeline' bullet list only when pipeline states are available, and a final verification note when any value is conflicting or unverified.",
